...
const FadeScaleImage = ({ src, index }: { src: string; index: number }) => {
  ...
  const progress = Math.max(0, Math.min(1, (frame - startFrame) / fps));

  // Interpolate opacity and scale values
  ...
  return isVisible ? (
    <img
      ...
      style={{
        ...
        transition: "opacity 0.4s ease-in-out, transform 0.4ss ease-in-out",
      }}
    />
  ) : null;
};
  ...
  return (
    <>
      {images.map((src, index) => (
        <FadeScaleImage src={src} index={index} />
      ))}
    </>
  );
};
